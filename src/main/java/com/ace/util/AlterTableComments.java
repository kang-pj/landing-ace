...
public class AlterTableComments {
    
    public static void main(String[] args) {
        System.out.println("테이블 코멘트 업데이트 시작...");
        
        String[] sqlStatements = {
            // traffic_logs 테이블 코멘트
        ...
        try (Connection conn = DatabaseUtil.getConnection();
            ...
            for (String sql : sqlStatements) {
                System.out.println("실행 중 [" + (++count) + "/" + sqlStatements.length + "]: " + 
                    sql.substring(0, Math.min(60, sql.length())) + "...");
                stmt.execute(sql);
            }
            
            System.out.println("\n✓ 테이블 코멘트 업데이트 완료!");
            System.out.println("✓ traffic_logs 테이블 및 컬럼 코멘트 업데이트됨");
            System.out.println("✓ consultation_leads 테이블 및 컬럼 코멘트 업데이트됨");
            
        } catch (Exception e) {
            System.err.println("✗ 코멘트 업데이트 실패: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
